	...
	public void write(List<? extends Location> locations) throws Exception {
		for(Location location : locations)
		{
			...
			try {
				...
				state_id = jdbcTemplate.queryForObject("SELECT STATE_ID FROM NE_STATE_MASTER WHERE VC_STATE = '" + location.getState() + "' AND VC_COUNTRY = " + country_id + "", Integer.class);
			    
				int city_id = jdbcTemplate.queryForObject("SELECT CITY_ID FROM NE_CITY_MASTER WHERE VC_CITY = '" + location.getCity() + "' AND IN_STATE = " + state_id + "", Integer.class); 
			...
			{
				jdbcTemplate.update("INSERT INTO NE_CITY_MASTER (VC_CITY, IN_STATE, IN_STATUS) VALUES ('" + location.getCity() + "', " + state_id + ", 13)");
